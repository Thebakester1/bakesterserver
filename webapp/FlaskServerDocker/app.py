...
def connection():
    ...
    cstr = 'DRIVER=ODBC Driver 17 for SQL Server;SERVER='+s+';PORT:'+port+';DATABASE='+d+';UID='+u+';PWD='+ p
    conn = pyodbc.connect(cstr)
    return conn
...
#         billamount = float(request.form["billamount"])
#         conn = connection()
#         cursor = conn.cursor()
#         cursor.execute("INSERT INTO dbo.Orders (Order_ID, Date, Customer_ID, Product_ID, Description, BillAmount) VALUES (?, ?, ?, ?, ?, ?)", order_id, date, customer_id, product_id, description, billamount )
#         conn.commit()
#         conn.close()
#         return redirect('/oms')
